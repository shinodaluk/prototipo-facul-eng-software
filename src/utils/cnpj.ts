const  strip = (cnpj: string) =>  (cnpj || "").toString().replace( /[^\d]/g , "");


const digit = (numbers: string) => {
    let index = 2
...
const format = (cnpj: string) =>  strip(cnpj).replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/, "$1.$2.$3/$4-$5");
...
export const gerarCNPJ = (opts: { format?: (val: string) => string } = {} ) => {
    let cnpj = ''
    let i = 12
