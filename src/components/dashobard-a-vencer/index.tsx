...
export default function EmprestimosAVencer() {
    ...
    const rows = useMemo(() => {
        return emprestimoGeral.filter((equip) => equip.id_empre === empresa && !equip.status && isBefore(new Date(), new Date(equip.data_dev))).
            filter((equip) => {
                if (userType === "adm") return true;
                if (userType === "geral") {
                    return usuariosEmprestimosGeral.some(
                        (usu_emp) => usu_emp.id_usug === user && usu_emp.id_emp === equip.id_emp
                    );
                }
                return false;
            });
    ...
    return (
        <Grid width="50%" spacing={2}>
            <Typography variant="h5">Empréstimos a vencer</Typography>
            <DataGrid
                rows={rows}
                columns={columns}
                getRowId={getRowId}
